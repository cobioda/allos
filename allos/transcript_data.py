...
# %% auto 0
__all__ = ['BuildSpliceGraph', 'FindBubbles', 'CheckOverlap', 'ClassifyBubble', 'TranscriptData']
...
def BuildSpliceGraph(inputDF: DataFrame) -> DiGraph:
    """
    inputDF has columns:
      ['transcript_ID','chrom','strand','exonStart','exonEnd']
    where exonStart/exonEnd are lists of ints.
    """
    G = DiGraph()

    # 1) Add every boundary as a node with the right attribute
    starts = set(sum(inputDF['exonStart'].tolist(), []))
    ends   = set(sum(inputDF['exonEnd'].tolist(),   []))
    for s in starts:
        G.add_node(s, attribute='start')
    for e in ends:
        G.add_node(e, attribute='end')
    G.add_node('root', attribute='root')
    G.add_node('sink', attribute='sink')

    # 2) Build edges: exon edges, intron edges, plus root/sink connectors
    exon_edges, intron_edges, aux_edges = [], [], []
    for _, row in inputDF.iterrows():
        xs, xe = row['exonStart'], row['exonEnd']
        # root → first start, last end → sink
        aux_edges.append(('root', xs[0]))
        aux_edges.append((xe[-1], 'sink'))
        # exon edges
        exon_edges += list(zip(xs, xe))
        # intron edges
        intron_edges += list(zip(xe[:-1], xs[1:]))

    G.add_edges_from(exon_edges)
    G.add_edges_from(intron_edges)
    G.add_edges_from(aux_edges)

    return G

def FindBubbles(G: DiGraph):
    bubbles = []
    cur, start = 'root', 'root'
    while start != 'sink':
        if G.in_degree(cur) == 2:
            bubbles.append(list(all_simple_paths(G, source=start, target=cur)))
        if G.out_degree(cur) == 2:
            start = cur
            cur   = next(G.successors(cur))
        elif G.out_degree(cur) == 1:
            cur = next(G.successors(cur))
        else:
            start = cur
    return bubbles
...
def ClassifyBubble(bubble, attrs):
    p1, p2 = bubble
    L1, L2 = len(p1), len(p2)

    # SE / RI
    if {L1, L2} == {2,4}:
        small = p1 if L1 < L2 else p2
        a0, aN = attrs[small[0]], attrs[small[-1]]
        coords = sort(list(set(p1)^set(p2)))
        if (a0, aN) == ('start','end'):
            return 'RI', ":".join(map(str,coords))
        if (a0, aN) == ('end','start'):
            return 'SE', ":".join(map(str,coords))

    # A5SS / A3SS
    if L1==3 and L2==3:
        a0, aN = attrs[p1[0]], attrs[p1[-1]]
        if (a0, aN) == ('start','start'):
            c1 = sort(p1[1:3]); c2 = sort(p2[1:3])
            return 'A5SS', f"{':'.join(map(str,c1))};{':'.join(map(str,c2))}"
        if (a0, aN) == ('end','end'):
            c1 = sort(p1[-3:-1]); c2 = sort(p2[-3:-1])
            return 'A3SS', f"{':'.join(map(str,c1))};{':'.join(map(str,c2))}"

    # MXE / AFE / ALE
    if L1==4 and L2==4:
        s1, e1 = attrs[p1[0]], attrs[p1[-1]]
        m1, m2 = sort(p1[1:3]), sort(p2[1:3])
        if s1=='end' and e1=='start' and not CheckOverlap(m1,m2):
            return 'MXE', f"{':'.join(map(str,m1))};{':'.join(map(str,m2))}"
        if s1=='root' and e1=='start':
            return 'AFE', f"{':'.join(map(str,m1))};{':'.join(map(str,m2))}"
        if s1=='end' and e1=='sink':
            return 'ALE', f"{':'.join(map(str,m1))};{':'.join(map(str,m2))}"
...
class TranscriptData:
    ...
    def compare_transcript_isoforms(self,
        ...
        """
        Compare alternative splicing events between transcript isoforms
        using splice‐graph bubbles (Wang 2022, Xing Lab).  Supports:

        SE, MXE, A5SS, A3SS, RI, AFE, ALE, COMPLEX
        ...
        if transcripts is None and gene_id is None:
            logging.error("Need transcripts or gene_id.")
            return pd.DataFrame()
        if gene_id:
            transcripts = self.get_transcripts_by_gene_name(gene_id) or []
        if len(transcripts)<2:
            logging.error("Need at least two transcripts.")
            return pd.DataFrame()

        # 2) pick reference
        if reference_transcript is None:
            reference_transcript = transcripts[0]
        elif reference_transcript not in transcripts:
            transcripts.insert(0, reference_transcript)

        results = []
        for alt in transcripts:
            if alt == reference_transcript:
                continue

            # build the 2-row DataFrame
            rows = []
            for tx in (reference_transcript, alt):
                exdf  = self.get_exons(tx).df
                chrom = exdf["Chromosome"].iat[0]
                strand= self.get_intron_ranges(tx).df.columns.name  # or however you store strand
                rows.append([tx, chrom, strand,
                            exdf["Start"].tolist(),
                            exdf["End"].tolist()])
            pairDF = DataFrame(rows, columns=
                    ["transcript_ID","chrom","strand","exonStart","exonEnd"])

            # splice‐graph + bubble‐find
            sg       = BuildSpliceGraph(pairDF)
            attrs    = get_node_attributes(sg, "attribute")
            bubbles  = FindBubbles(sg)

            # classify each bubble
            evs, dets = [], []
            for bub in bubbles:
                evt, coord = ClassifyBubble(bub, attrs)
                evs.append(evt)
                dets.append(coord)

            # collapse if asked
            if collapse_events and len(evs)>1:
                label, detail = "Complex", "; ".join(dets)
            else:
                label, detail = ", ".join(evs), "; ".join(dets)

            results.append({
                "gene_id":                gene_id or "",
                "reference_transcript":   reference_transcript,
                "alternative_transcript": alt,
                "events":                 label,
                "details":                detail
            })
