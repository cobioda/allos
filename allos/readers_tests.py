...
# %% auto 0
__all__ = ['file_path', 'get_resource_path', 'download_test_data', 'iso_concat', 'read_sicelore_isomatrix']
...
def get_resource_path(filename):
    ...
    # Detect if running inside a script (__file__ exists) or inside a Jupyter notebook
    if "__file__" in globals():
        # Running from library code (`allos/`)
        project_root = Path(__file__).resolve().parent.parent  # Go up two levels
    else:
        # Running inside a notebook (`nbs/`)
        project_root = Path.cwd().parent  # Go up one level

    # Construct the full path to the file inside resources/
    resource_path = project_root / "resources" / filename

    # Check if the file exists
    if not resource_path.exists():
        raise FileNotFoundError(f"❌ File not found at: {resource_path}")
